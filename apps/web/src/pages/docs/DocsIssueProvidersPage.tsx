...
export function DocsIssueProvidersPage() {
  return (
    ...
    >
      <p>
        Each project points at one GitHub repository, GitLab project, or Linear team (or Linear
        project). Authentication is a personal access token (GitLab), a GitHub App installation /
        personal access token (GitHub), or a personal API key (Linear). The token or app needs
        permission to create issues, post comments, list labels and members, and register a webhook
        for comment events.
      </p>

      <h2>Paste a link in the wizard</h2>
      <p>
        When creating a project, you can paste a tracker URL and ServiceBeard detects the provider
        automatically:
      </p>
      <ul>
        <li>
        ...
        <li>
          ...
          <code>https://linear.app/acme/team/ENG/active</code>) or project URL (for example{" "}
      ...
      <div className={styles.cardGrid}>
        ...
        <Link to={DOC_PATHS.linear} className={styles.card}>
          ...
          <p className={styles.cardDesc}>
            Personal API keys for Linear workspaces — link a team or project on linear.app.
          </p>
        </Link>
      </div>
      ...
      <p>
        This works on GitHub, GitLab, and Linear. On GitLab you can also use native internal notes;
        both are skipped for outbound email.
      </p>
      <p>
