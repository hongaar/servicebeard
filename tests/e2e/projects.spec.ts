...
import { E2E_PROJECTS, E2E_RULES, E2E_TEAMS, E2E_THREAD } from "./fixtures/constants";
...
test.describe("Projects and resources UI flow", () => {
  ...
  test("Admin can view seeded team projects, rules, and conversation threads", async ({
    ...
    await expect(page).toHaveURL(new RegExp(`/teams/${data.teams.teamA.id}/projects`));
    await expect(page.getByText(E2E_PROJECTS.projectA.name)).toBeVisible();
    await page.getByRole("link", { name: `Open ${E2E_PROJECTS.projectA.name}` }).click();

    await expect(page).toHaveURL(
      new RegExp(`/teams/${data.teams.teamA.id}/projects/${data.projects.projectA.id}`),
    );
    ...
    await expect(page.getByText(E2E_THREAD.threadA.subjectNormalized)).toBeVisible();
  });
