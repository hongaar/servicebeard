...
test.describe("Global search UI", () => {
  test("Search returns only resources from the signed-in user's teams", async ({ page }) => {
    const data = seed();
    ...
    await expect(searchDialog.getByRole("button", { name: /E2E Team A/i }).first()).toBeVisible();
    await expect(searchDialog.getByText(E2E_TEAMS.teamB.name)).not.toBeVisible();
  });
});
